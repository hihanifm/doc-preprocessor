...
- Users can **Save for developer** in the UI: the file lands under **`SUPPORT_UPLOAD_DIR`** (default **`support_uploads/`** next to `app.py`) with a unique **`reference`** filename the developer can look up (nothing is auto-added to `samples/`).
